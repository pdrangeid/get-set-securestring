...
.DESCRIPTION 
 ...
 (or with access to) it. 
  See this article: https://github.com/dotnet/runtime/issues/30612
 
 If you have access to the scripts, and some level of local administrative privilleges,
 it is a trivial task to alter the scripts in order to recover/retrieve the original text of the stored
 securestring values.  Also once retrieved it may be in memory and vulnerable to in-memory retrieval.
 
 As best security practices demand, these stored credentials should only provide the least
 ...
 Consider this solution a good step better than obfuscation, but IT IS NOT PERFECT!
...
Catch{
    ...
    if ($base64 -eq $true){#base64 combines the username:password strings and converts to base64 before hashing into a securestring
        $UserVal = Ver-RegistryValue -RegPath $credpath -Name $creduser
        $thePW = Get-SecurePassword $credpath $credpw 
        $pair = $pair = "$($UserVal):$($thePW)"
        write-host "the pair is $pair"
        $base64Creds = -join("Basic ",[System.Convert]::ToBase64String([System.Text.Encoding]::ASCII.GetBytes($pair)))
        write-host "setting using set-securepassword $credpath $creduser $credpw 'username' $base64Creds"
        Set-SecurePassword $credpath $creduser $credpw "username" $base64Creds
        
        #Set-SecurePassword([String]$pwpath,[String]$UsrValName,[String]$PWValName,[String]$theusername,[String]$UnsecurePW){
    } 
